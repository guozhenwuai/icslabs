...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
	...
	case A_ADD:
		...
		if((argA>0&&argB>0&&val<=0)||(argA<0&&argB<0&&val>=0))ovf = TRUE;
		break;
	case A_SUB:
		if(val==0)zero = TRUE;
		else if(val<0)sign = TRUE;
...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
	...
	case C_YES:
		...
		break;
	case C_LE:
		if(GET_ZF(cc)|(GET_SF(cc)^GET_OF(cc)))doit = TRUE;
		break;
	case C_L:
		if(GET_SF(cc)^GET_OF(cc))doit = TRUE;
		break;
	case C_E:
		if(GET_ZF(cc))doit = TRUE;
		break;
	case C_NE:
		if(!GET_ZF(cc))doit = TRUE;
		break;
	case C_GE:
		...
		break;
	case C_G:
		if((~(GET_SF(cc)^GET_OF(cc))&~GET_ZF(cc))&1)doit = TRUE;
		break;
	default:
		break;
	}
...
stat_t nexti(y86sim_t *sim)
{
	...
	byte_t rArB = 0;
	regid_t regA =-1,regB = -1;
	long_t valA = 0,valB = 0,valE = 0,valM = 0,imm = 0;
	...
	switch (icode) {
		...
		case I_RRMOVL:  /* 2:x regA:regB */
			if((cond_t)ifun<C_YES||(cond_t)ifun>C_G){
				err_print("PC = 0x%x, Invalid instruction %.2x", sim->pc, codefun);
				return STAT_INS;
			}
			if(!get_byte_val(sim->m,next_pc,&rArB)){
				err_print("PC = 0x%x, Invalid instruction address", next_pc);
				return STAT_ADR;
			}
			regA = GET_REGA(rArB);
			regB = GET_REGB(rArB);
			if(!NORM_REG(regA)||!NORM_REG(regB)){
				err_print("PC = 0x%x, Invalid instruction %.2x", next_pc, rArB);
				return STAT_INS;
			}
			next_pc++;
		...
		case I_RMMOVL: /* 4:0 regA:regB imm */
			...
			if(!set_long_val(sim->m, valE, valA)){
				err_print("PC = 0x%x, Invalid instruction address", valE);
				return STAT_ADR;
			}
			sim->pc = next_pc;
			break;
		...
		case I_JMP: /* 7:x imm */
			...
			next_pc+=4;
			if(cond_doit(sim->cc,(cond_t)ifun))sim->pc = imm;
			else sim->pc = next_pc;
			break;
		case I_CALL: /* 8:x imm */
			if(!get_long_val(sim->m,next_pc,&imm)){
		...
		case I_RET: /* 9:0 */
			...
			if(!get_long_val(sim->m,valA,&valM)){
				err_print("PC = 0x%x, Invalid instruction address", valA);
				return STAT_ADR;
			}
			set_reg_val(sim->r,REG_ESP,valE);
			sim->pc = valM;
